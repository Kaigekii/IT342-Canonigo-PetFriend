...
export default function PetOwnerMessagesPage() {
  ...
  const loadData = useCallback(async (authToken) => {
    ...
    (Array.isArray(bookingsData) ? bookingsData : []).forEach((booking) => {
      if (!booking.sitterId) return;
      if (!contactMap.has(booking.sitterId)) {
        const thread = threadMap.get(booking.sitterId);
        contactMap.set(booking.sitterId, {
          id: booking.sitterId,
  ...
  return (
    <div style={styles.page}>
      ...
      <main style={styles.content}>
        ...
        <div style={styles.helperText}>You can only message sitters you have booked before.</div>
        {error && <div style={styles.errorBox}>{error}</div>}

        {loading ? (
          <div style={styles.emptyBox}>Loading conversations...</div>
        ) : conversations.length === 0 ? (
          <div style={styles.emptyBox}>No bookings yet. Book a sitter to start messaging.</div>
        ) : (
            ...
            <section style={styles.chatPane}>
              ...
              <div style={styles.messageArea}>
                {!activeConversation ? (
                  ...
                  <div style={styles.emptyBox}>Start your conversation with this sitter.</div>
                ) : (
                  messages.map((msg) => (
                    <div
                      key={msg.messageId}
